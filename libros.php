...
if (isset($_GET["id"])) {
  if (($_GET["id"] == $_SESSION["id"]) OR ($_SESSION["tipo"] == "Admin")) {
    $result = getListRentados($_GET["id"]);
  }else{
    $result = false;
  }
}else{
  $result = getAllRentados();
}
              ...
              <tbody>
                <?php while($row = $result->fetch_assoc()){ ?>
                  <tr>
                    ...
                    <td>
                      <a href="libros.php?id=<?php echo $row['IdUsuario'] ?>">
                      <?php echo $row["Nick"] ?>
                    </a>
                    </td>
                    <td class="cantidad">
            ...
            <?php else: ?>
              ...
              while ($row = $result->fetch_assoc()) {
                ...
                <div class="col-sm-6 col-md-2 col-lg-2 col-xs-6">
                  ...
                  <div class="caption">
                    <h4 class="text-center"><?php echo $row['Titulo']; ?> </h4><br>
                    <p class="alert alert-warning">
                      Rentado durante <?php echo $row['diasr'] ?> <?php echo ($row['diasr'] == 1)? "dia":"dias" ?>
                    </p>
                    <p>
                      <?php if ($admin): ?>
                        <a href="controller/entregar.php?id=<?php echo $row['IdPedido']?>" class="btn btn-success">Entregar</a>
                      <?php endif ?>
                    </p>
                  </div>
                </div>
